...
names(powerTables) <- c("2019", "2020", "2021", "2022", "2023")
...
for(year in names(powerTables)){
  ...
  sept.start <- as.POSIXct(paste0(year, "-01-01 00:00:00"), format = "%Y-%m-%d %H:%M:%S")
  sept.end <- as.POSIXct(paste0(year, "-12-31 24:00:00"), format = "%Y-%m-%d %H:%M:%S")
...
optCutOff19 <- get.CutOffvsDay(powerTables[["2019"]], "2019-01-01 00:00:00", "2019-12-31 00:00:00" )
optCutOff19
optCutOff20 <- get.CutOffvsDay(powerTables[["2020"]], "2020-01-01 00:00:00", "2020-12-31 00:00:00")
optCutOff20
optCutOff21 <- get.CutOffvsDay(powerTables[["2021"]], "2021-01-01 00:00:00", "2021-12-31 00:00:00")
optCutOff21
optCutOff22 <-  get.CutOffvsDay(powerTables[["2022"]], "2022-01-01 00:00:00", "2022-12-31 00:00:00")
optCutOff22
optCutOff23 <-  get.CutOffvsDay(powerTables[["2023"]], "2023-01-01 00:00:00", "2023-12-31 00:00:00")
optCutOff23


optCutOff20_23 <- rbind(optCutOff20, optCutOff21, optCutOff22, optCutOff23)
optCutOff20_23

write.csv(optCutOff20_23, "optCutOff20_23", row.names = FALSE)
...
for (i in seq_along(files)) {
  ...
  start.date <- as.POSIXct(paste0(years[i], "-01-01 00:00:00"), format = "%Y-%m-%d %H:%M:%S")
  end.date <- as.POSIXct(paste0(years[i], "-12-31 24:00:00"), format = "%Y-%m-%d %H:%M:%S")
...
temps.avg.all <- read.csv("Avg.Temps.19_23")

write.csv(temps.avg.all, "Avg.Temps.19_23", row.names = FALSE)
...
plot(table2021[[2]], table2021[[3]], col = "blue", ylim = c(0,3), xlim = c(0,32), xlab = "Temp C", ylab = "Optimal Cutoff (Daily) [MW]")
par(new=TRUE) 
plot(table2022[[2]], table2022[[3]], col = "green", ylim = c(0,3), xlim = c(0,32), xlab = "Temp C", ylab = "Optimal Cutoff (Daily) [MW]")
par(new=TRUE) 
plot(table2023[[2]], table2023[[3]], col = "purple", ylim = c(0,3), xlim = c(0,32), xlab = "Temp C", ylab = "Optimal Cutoff (Daily) [MW]")
par(new=TRUE) 
plot(table2020[[2]], table2020[[3]], col = "red", ylim = c(0,3), xlim = c(0,32), xlab = "Temp C", ylab = "Optimal Cutoff (Daily) [MW]")
